...
namespace Parallax.Gameplay.Rooms
{
    public sealed class DoorRetreatTrap : RoomTrap
    {
        ...
        protected override void Awake() { base.Awake(); if (trigger == null && UsesOverlapSource && !IsPeriodic) Debug.LogError($"{GetType().Name} on '{name}': missing Trigger", this); if (doorRoot == null) Debug.LogError($"{GetType().Name} on '{name}': missing doorRoot", this); if (Reality == null) Debug.LogError($"{GetType().Name} on '{name}': missing RealityRoot", this); if (observers == null) Debug.LogError($"{GetType().Name} on '{name}': missing ObserverSet", this); if ((trigger == null && UsesOverlapSource && !IsPeriodic) || doorRoot == null || Reality == null || observers == null) { enabled = false; return; } filter = new ContactFilter2D { useLayerMask = true, layerMask = Reality.PhysicsMask, useTriggers = false }; countdown = new TrapCountdown(delayTicks); start = doorRoot.position; }
