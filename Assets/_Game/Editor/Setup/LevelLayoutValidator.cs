...
namespace Parallax.Editor.Setup
{
    ...
    public static partial class LevelLayoutValidator
    {
        ...
        // PAX-075 (D-079): the route rule. Deliberately not part of Validate(): it replays the level's declared
        // solution and betrayal routes (LevelRoutes) through the real game code in a swapped-in scene.
        public static List<string> ValidateRoutes(string levelId, SoloRoomDefinition room)
        {
            if (!Levels.LevelRoutes.TryGet(levelId, out Routes.RoomRoutes routes))
                return new List<string> { $"{levelId}: no routes are declared in LevelRoutes (D-069 (1), D-079)." };
            return ValidateRoutes(levelId, room, routes);
        }

        public static List<string> ValidateRoutes(string levelId, SoloRoomDefinition room, Routes.RoomRoutes routes) =>
            Routes.RouteValidator.Validate(levelId, room, routes);
