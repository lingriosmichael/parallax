...
- Levels 1–10 (band 1, PAX-059; D-085 lands with half B) pass `LevelLayoutValidator.Band1.cs` and `Band1LevelTests`: lethal traps and a sequential chain, dead ends declared as Dies routes named "Dead end…" or as Recovers routes, width ≤ 32, and a 500–1500-tick solution (L001 ≥ 300). By convention (no check enforces it), every mid-air release on a solution is written as a timed step, so the route validator's windows sweep measures it (window ≥ 12).
