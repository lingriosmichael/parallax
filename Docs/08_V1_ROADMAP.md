...
| ✓ | PAX-079 | Coyote time and jump buffer in whole ticks (row 4c): `JumpWindows` in `Parallax.Core`, lengths from `TickTime.ToWholeTicks` (round half up), 5 and 6 ticks at 50 Hz as since abb55f9; equivalence test against the old float logic at the committed step. | D-077 |
